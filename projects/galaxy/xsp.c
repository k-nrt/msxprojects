...
#define SPRITE_HALF_SIZE (8)
#define SPRITE_Y_MASK  (0x7)
#define SPRITE_Y_SHIFT (3)

#endif

#if (SPRITE_SIZE == 32)

#define SPRITE_HALF_SIZE (16)
#define SPRITE_Y_MASK  (0xf)
#define SPRITE_Y_SHIFT (4)
...
#define SPRITE_COUNTER_MAX (256/SPRITE_HALF_SIZE)
static u8 s_nCounter[SPRITE_COUNTER_MAX];
...
void SpbSetSprite(u8 iPlane, s16 x, s16 y, u8 iPattern, u8 nColor)
{
	...
	if( x < -SPRITE_SIZE || 255 < x || y < -SPRITE_SIZE || 191 < y )
	{
		//. クリップ.
		*pVdp = 209;
		pVdp += 5;
...
void SpbUpdate(void)
{
	...
	if( phase == 0 )
	{
		...
		pVdp = s_nCounter;
		for( i = 0 ; i < SPRITE_COUNTER_MAX ; i++, pVdp++ )
		{
			*pVdp = 0;
		}
